    ...
    for (const dateDir of dateDirs) {
        ...
        for (const csvFile of csvFiles) {
            ...
            for (const row of rows) {
                ...
                const counterKey = `${parsed.birdid}_${parsed.yyMMdd}_${safeLabel}`;
                const id = nextId(counterKey);
                const idStr = String(id).padStart(6, '0');
                const outName = `${parsed.birdid}_${parsed.yyMMdd}_${safeLabel}.${idStr}.wav`;
                const outPath = join(labelsDir, outName);
