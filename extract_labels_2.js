...
function parseWavBaseName(csvFileName) {
    ...
    const m = wavName.match(/^([^_]+)_(\d{6})_/);
    if (!m) return null;
    return {
        wavFileName: wavName,
        birdid: m[1],
    ...
    for (const dateDir of dateDirs) {
        ...
        for (const csvFile of csvFiles) {
            ...
            for (const row of rows) {
                ...
                const labelDir = join(labelsDir, safeLabel);
                if (!existsSync(labelDir)) {
                    mkdirSync(labelDir, { recursive: true });
                }

                const counterKey = `${safeLabel}/${parsed.birdid}_${parsed.yyMMdd}`;
                const id = nextId(counterKey);
                const idStr = String(id).padStart(6, '0');
                const outName = `${parsed.birdid}_${parsed.yyMMdd}.${idStr}.wav`;
                const outPath = join(labelDir, outName);
